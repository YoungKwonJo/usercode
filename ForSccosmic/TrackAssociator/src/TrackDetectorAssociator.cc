...
// $Id: TrackDetectorAssociator.cc,v 1.38 2009/05/26 08:33:05 fabiocos Exp $
//
//
						    ...
						    const AssociatorParameters& parameters)
{
   ...
   for(std::set<DetId>::const_iterator detId = muonIdsInRegion.begin(); detId != muonIdsInRegion.end(); detId++)
   {
       ...
       if (this->isclose(*detId, match, parameters))
       {
           // RPC chamber -- Added by Suyong Choi - get RPC rolls
           if(const RPCChamber* rpcChamber = dynamic_cast<const RPCChamber*>(geomDet) ) {
             //  LogTrace("TrackAssociator") << "matching RPC chamber has " <<  rpcChamber->nrolls() << " rolls";
               const std::vector<const RPCRoll*> rpcrolls = rpcChamber->rolls();
               for (std::vector<const RPCRoll*>::const_iterator iroll=rpcrolls.begin();
                      iroll != rpcrolls.end(); iroll++)
               {
                  // Surface::PositionType locpos=(*iroll)->position();
                   //printf("%6.2f %6.2f %6.2f\n", locpos.x(), locpos.y(), locpos.z());
                   TAMuonChamberMatch matchroll;
                   if (this->isclose((*iroll)->id(), matchroll, parameters)) {
                       matches.push_back(matchroll);// push back the roll level
                   }
               }
           }
           else if (const DTChamber* dtChamber = dynamic_cast<const DTChamber*>(geomDet) ) {
                LogTrace("TrackAssociator") << "This is a DT chamber, looking for layers "<< detId->rawId();
                 const std::vector<const DTSuperLayer *> dtsuperlayers = dtChamber->superLayers();
                 // loop over superlayers
                 for (std::vector<const DTSuperLayer *>::const_iterator idtsl= dtsuperlayers.begin(); idtsl!=dtsuperlayers.end(); idtsl++)
                 {
                     const std::vector< const DTLayer * > dtlayers = (*idtsl)->layers();
                     // loop over layers
                     for (std::vector<const DTLayer *>::const_iterator idtl= dtlayers.begin(); idtl!=dtlayers.end(); idtl++)
                     {
                           TAMuonChamberMatch matchdtlayer;
                           if (this->isclose((*idtl)->id(), matchdtlayer, parameters)) {
                               matches.push_back(matchdtlayer);// push back the roll level
                           }
                     }
                 }
           }
           else if (const CSCChamber* cscChamber = dynamic_cast<const CSCChamber*>(geomDet) ) { // CSC
                LogTrace("TrackAssociator") << "This is a CSC chamber, looking for layers "<< detId->rawId();
                 const std::vector< const CSCLayer * > csclayers = cscChamber->layers();
                 // loop over layers
                 for (std::vector<const CSCLayer *>::const_iterator icscl= csclayers.begin(); icscl!=csclayers.end(); icscl++)
                 {
                       TAMuonChamberMatch matchcsclayer;
                       if (this->isclose((*icscl)->id(), matchcsclayer, parameters)) {
                           matches.push_back(matchcsclayer);// push back the roll level
                       }
                 }
           }
       }
   ...
   for(std::vector<TAMuonChamberMatch>::iterator matchedChamber = matchedChambers.begin(); 
	...
	if(const DTChamber* chamber = dynamic_cast<const DTChamber*>(geomDet) ) {
	   ...
	   // CSC Chamber
        ...
        else if ( const RPCChamber* chamber = dynamic_cast<const RPCChamber*>(geomDet) ) {
        }
        else if ( const RPCRoll* chamber = dynamic_cast<const RPCRoll*>(geomDet) ) {
        }
        else if ( const DTLayer* chamber = dynamic_cast<const DTLayer*>(geomDet) ) {
        }
        else if ( const CSCLayer* chamber = dynamic_cast<const CSCLayer*>(geomDet) ) {
        }
        else{
...
bool TrackDetectorAssociator::isclose(const DetId &detId, TAMuonChamberMatch &match
                        , const AssociatorParameters& parameters)
{
    //SteppingHelixStateInfo trajectoryPoint = cachedTrajectory_.getStateAtHcal();
    SteppingHelixStateInfo trajectoryPoint = cachedTrajectory_.getOuterState();
    // If trajectory point at HCAL is not valid, try to use the outer most state of the
    // trajectory instead.
    //if(! trajectoryPoint.isValid() ) trajectoryPoint = cachedTrajectory_.getOuterState();
    if(! trajectoryPoint.isValid() ) trajectoryPoint = cachedTrajectory_.getInnerState();
    if(! trajectoryPoint.isValid() ) {
        LogTrace("TrackAssociator") <<
        "trajectory position at Innerstate is not valid. Assume the track cannot reach muon detectors and skip it";
        return false;
    }
    ...
    if (! stateOnSurface.isValid()) {
     LogTrace("TrackAssociator") << "Failed to propagate the track; moving on\n        "<<
        ...
        stateOnSurface = cachedTrajectory_.propagate( &geomDet->surface() );
        cachedTrajectory_.setPropagationStep( currentStepSize );

        if (! stateOnSurface.isValid()) {
         LogTrace("TrackAssociator") << "Failed to propagate the track in the reverse direction as well; moving on\n        "<<
            detId.rawId() << " not crossed\n";
          return false;
        }
    }
    ...
    {
        LogTrace("TrackAssociator") << "Succeeded to propagate the track to "<< detId.rawId();
    }
